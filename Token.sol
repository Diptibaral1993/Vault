...
interface governaces{
    function proposal(address wallet, uint256 _amount,address _tokenaddress) external;
...
contract TestToken is IERC20,Ownable {
    ...
    function _transfer(
        ...
        INFvault(vaultaddress).setReward(from,_balances[from],tokentotalSupply);
        INFvault(vaultaddress).setReward(to,_balances[to],tokentotalSupply);
    ...
    function setVault(address _vault) public onlyOwner returns(bool){
        require(_vault!=address(0),"Invalid Address");
        vaultaddress=_vault;
        return true;
    }
    
    function burnbyproposal(uint proposalid) public onlyOwner returns(bool,address,uint){
         require(governanceaddress != address(0), "Governance not set");
         (bool pass,address walletaddress,uint amount)=governaces(governanceaddress).proposalStatus(proposalid);
         _burn( walletaddress, amount);
         return (pass,walletaddress,amount);

    }
    
    function setgovernace(address  _governance)public onlyOwner returns (bool){
